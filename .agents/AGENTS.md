 ...
 * </pre>
 */
* 신한라이프 인터페이스 통신 프로토콜 및 메시지 포맷 가이드 (구간별 메시지 포맷 및 프로토콜):
  * 처리계/단위 ↔ 대외MCI 구간: 무조건 `HTTPS` / `FixedLength`를 사용한다.
  * 그 외 주요 구간(채널시스템 ↔ 대내MCI, 대내MCI ↔ 처리계/단위, 처리계/단위 ↔ EAI)은 `HTTPS` / `JSON`을 사용한다.
